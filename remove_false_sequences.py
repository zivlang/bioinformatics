        ...
        for species, hits in hits_ranges.items():
            ...
            for index in range(len(hits)):
                if not hit_to_keep:
                    ...
                    continue
                # hit_to_keep engulfs hits[index]:
                if hits[index][0] >= hit_to_keep[0] and hits[index][1] <= hit_to_keep[1]:
                    continue
                elif hits[index][1] > hit_to_keep[1] > hits[index][0]:  # partial overlap
...
def delete_false_hits(hits, fasta_file):
    ...
    if len(hits) > 1:
        ...
        local_dictionary = [item for item in hits.values()]
    with open(f'{fasta_file[:-3]}_filtered.fa', 'w') as write:
        ...
        for header, sequence in zip(fasta_lines[::2], fasta_lines[1::2]):
            for scaffold, ranges in local_dictionary.items():
                if scaffold == header[1:-1]:
                    if len(ranges) > 1:
                        # If 'scaffold' in 'local dictionary' matches 'header' variable from the fasta file, the false
                        # sequences are assigned from 'sequence' by slicing it with the proper indexes in 'hit'.
                        hits_for_removal = [sequence[hit[0] - 1: hit[1] + 1] for hit in ranges if len(hit)]
                    else:
                        hits_for_removal = [sequence[ranges[0][0] - 1: ranges[0][1] + 1]]
                    ...
                    if len(kept_sequences) > 1:
                        for index, subsequence in enumerate(kept_sequences):
                            write.write(f'{header.strip()}_{index}\n')
                            write.write(f'{subsequence}\n')
                        cleaned = True
                    elif len(kept_sequences) == 1:
                        write.write(f'{header}')
                        write.write(f'{kept_sequences[0]}\n')
                        cleaned = True
            ...
            # if not cleaned:
            #     write.write(f'{header}')
            #     write.write(f'{sequence}')


def main():
    for file in os.listdir('blast'):
        if 'bttrB1' not in file or 'bwcaB1' not in file:
            continue
        ...
        delete_false_hits(merged_scaffold_ranges)
